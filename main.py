...
def load_data():
    ...
    if not EXCEL_PATH.exists():
        # Fallback: load the prebuilt normalized dataset persisted as a pickle
        if DATA_PATH.exists():
            log.info("📊 Excel missing → loading cached dataset from store_df.pkl")
            df = pd.read_pickle(DATA_PATH)
            with data_lock:
                store_df = df
                LAST_HASH = compute_hash(df)
            log.info(f"✅ Loaded {len(df)} stations (cached)")
            return
        log.error("❌ Excel file missing and no cached dataset found")
        return
...
def get_stations(
    ...
    with data_lock:
        if store_df is None: return {"features": [], "total": 0}
        df = store_df.copy()
